...
pub fn list_packages(_config: &Config, use_sudo: bool) -> Result<()> {
    ...
    match manager.list_installed() {
        Ok(packages) => {
            ...
            println!("{}", format!("Installed Packages ({}):", packages.len()).bold().cyan());
            ...
            if packages.len() > 50 {
                println!("\n  {} ... and {} more packages", "•".dimmed(), packages.len() - 50);
        ...
        Err(e) => {
            ...
            eprintln!("{}", "   If you're not on Fedora, package management is not available.".yellow());
        }
    }
...
"#.dimmed());
    ...
    println!("\n{}", format!("Declared Packages ({}):", config.packages.len()).bold().cyan());
...
pub fn compare_packages(config: &Config, use_sudo: bool) -> Result<()> {
    ...
    println!("{} Comparing declared vs installed packages...\n", "→".cyan());
    ...
    let extra_packages: Vec<_> = installed_map.keys()
    ...
    if !extra_packages.is_empty() {
        ...
        if extra_packages.len() > 10 {
            println!("  {} ... and {} more", "⊕".yellow(), extra_packages.len() - 10);
        }
    }
    ...
    if all_matched {
        println!("{} All declared packages are installed and match", "✓".green());
    } else {
        println!("{} Some packages are missing or have version mismatches", "⚠".yellow());
