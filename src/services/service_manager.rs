...
impl SystemdServiceManager {
    ...
    fn run_systemctl(&self, args: &[&str]) -> Result<()> {
        ...
        let output = cmd.output().map_err(|e| {
            DotfilesError::Path(format!(
                "Failed to execute systemctl: {}\n  💡 Make sure systemctl is installed",
                e
            ))
        })?;
    ...
    fn run_systemctl_output(&self, args: &[&str]) -> Result<String> {
        ...
        let output = cmd
            .output()
            .map_err(|e| DotfilesError::Path(format!("Failed to execute systemctl: {}", e)))?;
...
impl ServiceManager for SystemdServiceManager {
    fn is_enabled(&self, service: &str) -> Result<bool> {
        ...
        let output = cmd.output().map_err(|e| {
            DotfilesError::Path(format!("Failed to check if service is enabled: {}", e))
        })?;
    ...
    fn is_running(&self, service: &str) -> Result<bool> {
        ...
        let output = cmd.output().map_err(|e| {
            DotfilesError::Path(format!("Failed to check if service is running: {}", e))
        })?;
    ...
    fn status(&self, service: &str) -> Result<ServiceStatus> {
        ...
        let status_output = self
            .run_systemctl_output(&["status", service])
            .unwrap_or_default();
